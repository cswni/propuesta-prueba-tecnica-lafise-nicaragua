...
export function TransactionsList() {
  ...
  const { data, error, isLoading } = useGetAccountTransactionsQuery(String(accountId), {
    skip: !accountId,
  });
  ...
  if (error)
    return <div className="p-8 text-center text-red-500">Error al cargar transacciones</div>;
  if (!data) return null;
        ...
        <Tabs defaultValue="outline" className="w-full flex-col justify-start gap-6">
          <div className="flex items-center justify-between px-4 lg:px-6">
            ...
            <TabsList className="**:data-[slot=badge]:bg-muted-foreground/30 hidden **:data-[slot=badge]:size-5 **:data-[slot=badge]:rounded-full **:data-[slot=badge]:px-1 @4xl/main:flex">
              ...
              <TabsTrigger value="past-performance">Estado</TabsTrigger>
              <TabsTrigger value="key-personnel">Detalle</TabsTrigger>
              ...
              <Table>
                    ...
                    <TableRow key={headerGroup.id}>
                      {headerGroup.headers.map((header) => {
                        return (
                          <TableHead
                            className={
                              'py-4 font-semibold bg-transparent text-[var(--color-muted-foreground)]'
                            }
                            key={header.id}
                ...
                <TableBody className="**:data-[slot=table-cell]:first:w-8">
                  {table.getRowModel().rows?.length ? (
                    table
                      .getRowModel()
                      .rows.map((row) => <TransactionsTableRow key={row.id} row={row} />)
                  ) : (
                    <TableRow>
          ...
          <div className="relative w-full flex flex-col items-center pb-4 bg-gradient-to-r from-blue-600 to-[var(--green)] -mt-6 sm:mt-0">
            ...
            <div className="relative z-10 flex flex-col items-center">
              <img
                ...
                alt="Lafise Logo"
                className="w-32 h-16 sm:w-40 sm:h-20 mb-2 mt-3 drop-shadow-lg"
              />
          ...
          <div className="w-full max-w-md bg-white rounded-2xl p-6 mt-[-2em] px-4 py-2 z-10 mx-2 relative">
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-slate-900 font-bold text-lg text-right">
                  {item.description}
                </span>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-blue-700 font-bold text-lg text-right">
                  ${item.amount.value.toFixed(2)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="font-semibold text-green-700">Banco:</span>
                <span className="text-green-700 font-bold text-lg text-right">
